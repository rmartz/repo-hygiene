...
1. **Merge → release.** A merged `feat`/`fix` to `main` triggers semantic-release,
   which publishes a new `@rmartz/repo-hygiene` version to GitHub Packages and, via
   `scripts/set-hygiene-version.mjs`, bumps the `version` default inside
   `.github/workflows/hygiene.yml` in lockstep — committed into the release tag, so
   a given pinned ref installs a reproducible package version.
